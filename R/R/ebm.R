...
ebm_classify <- function(
   ...
   partials <- vector("list")
   for(col_name in col_names) {
      partials[[col_name]] <- vector("numeric", length(cuts[[col_name]]) + 1)
   }
   ...
   for(i_outer_bag in 1:outer_bags) {
      ...
      for(i_feature in 1:n_features) {
         partials[[col_names[i_feature]]] <- 
            partials[[col_names[i_feature]]] + result_list$model_update[[i_feature]]
      }
   }
   for(col_name in col_names) {
      partials[[col_name]] <- partials[[col_name]] / outer_bags
      # for now, zero all missing values
      partials[[col_name]][0] = 0
   }
   ...
   model <- structure(list(cuts = cuts, partials = partials), class = "ebm_model")
   return(model)
}
...
ebm_predict_proba <- function (model, X) {
   ...
   for(i_feature in 1:n_features) {
      ...
      partials <- model$partials[[col_name]]
      update_scores <- partials[discretized + 1]
...
ebm_show <- function (model, name) {
   ...
   y <- rep(partials[2:length(partials)], each = 2)
